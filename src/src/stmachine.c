...
int get_args(struct SysData* data)
{
	...
	{
		assert(data->argf);
		if (strlen(data->argf) <= MAX_FILE_NAME_SIZE && 
		    !containsSymbol(data->argf, '/') && !containsSymbol(data->argf, '\\') )
		{
...
int process_string(struct SysData* data)
{
	...
	if (delStrTeml(data->str, s_string_template) == TRUE)
	{
	...
	{
		// impossible come here with current process string logic 
		// printf("%s\n", MES_1_9);
		// return RET_FALSE;
	}
	return RET_FALSE;
